...
class AGI
{
    ...
    public function AGI($config = NULL, $optconfig = array()) {
        ...
        if ($this->request['agi_enhanced'] == '1.0') {
            if (file_exists('/proc/' . getmypid() . '/fd/3')) $this->audio = fopen('/proc/' . getmypid() . '/fd/3', 'r');
    ...
    public function exec($application, $options = '') {
        ...
        if ($GLOBALS['KINT_DUMP'] ) { 
            d("AGI->exec application :$application options: $options");
            return;
        }
    ...
    public function get_variable($variable, $get_value = false) {
        ...
        if ($GLOBALS['KINT_DUMP'] ) { 
            d("AGI->get_variable; variable: $variable");
            return;
        } 
    ...
    public function set_variable($variable, $value) {
        ...
        if ($GLOBALS['KINT_DUMP'] ) { 
            d("AGI->set_variable; variable: $variable value: $value");
            return;
        } 
    ...
    public function verbose($message, $level = 1) {
        ...
        if ($GLOBALS['KINT_DUMP'] ) { 
            d("AGI->verbose; message: $message level: $level");
            return;
        } 
    ...
    */
    ...
    public function espeak($text, $escape_digits = '', $frequency = 8000, $voice = NULL) {
        ...
        $delete = time() - 2592000;
         // 1 month
        foreach (glob($this->config['phpagi']['tempdir'] . DIRECTORY_SEPARATOR . 'espeak_*') as $file) if (filemtime($file) < $delete) unlink($file);
    ...
    public function which($cmd, $checkpath = NULL) {
        ...
        foreach (explode(':', $chpath) as $path) if (is_executable("$path/$cmd")) 
        return "$path/$cmd";
    ...
    public function make_folder($folder, $perms = 0755) {
        ...
        for ($i = 0; $i < count($f); $i++) {
            $base.= $f[$i];
            if ($f[$i] != '' && !file_exists($base)) mkdir($base, $perms);
